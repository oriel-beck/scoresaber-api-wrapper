...
export async function getPlayers(input: string | number = 1): Promise<SmallPlayer[]> {
    ...
    if (typeof input === 'string') {
        req = await petitio(`https://new.scoresaber.com/api/players/by-name/${input}`, 'GET').json<{players: smallplayerinfo[]} | apiError>()
    } else if (Number.isSafeInteger(input)) {
        req = await petitio(`https://new.scoresaber.com/api/players/${input}`, 'GET').json<{players: smallplayerinfo[]} | apiError>()
